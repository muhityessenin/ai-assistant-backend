...
func (f Feedback) MarshalJSON() ([]byte, error) {
	return json.Marshal(map[string]any{"id": f.ID, "assistant_id": f.AssistantID, "conversation_id": f.ConversationID, "message_id": f.MessageID, "user_id": f.UserID, "rating": f.Rating, "comment": f.Comment, "corrected_answer": f.CorrectedAnswer, "status": f.Status, "created_at": f.CreatedAt, "updated_at": f.UpdatedAt})
}
...
func (s *Service) Submit(ctx context.Context, a auth.Actor, messageID uuid.UUID, rating int, comment, corrected *string) (Feedback, error) {
	...
	var original, input string
	err := s.db.QueryRow(ctx, `SELECT c.assistant_id,c.id,m.content,(SELECT um.content FROM messages um WHERE um.organization_id=m.organization_id AND um.conversation_id=m.conversation_id AND um.role='user' AND um.created_at<m.created_at ORDER BY um.created_at DESC LIMIT 1) FROM messages m JOIN conversations c ON c.organization_id=m.organization_id AND c.id=m.conversation_id WHERE m.organization_id=$1 AND m.id=$2 AND m.role='assistant' AND ($3 OR c.user_id=$4)`, a.OrganizationID, messageID, a.IsAdmin(), a.UserID).Scan(&assistantID, &convID, &original, &input)
...
func (s *Service) List(ctx context.Context, a auth.Actor, page, limit int, assistantID, userID *uuid.UUID, rating *int, status string) ([]Feedback, int, error) {
	...
	rows, err := s.db.Query(ctx, `SELECT id,assistant_id,conversation_id,message_id,user_id,rating,comment,corrected_answer,status,created_at,updated_at,count(*) OVER() FROM message_feedback WHERE organization_id=$1 AND ($2::uuid IS NULL OR assistant_id=$2) AND ($3::uuid IS NULL OR user_id=$3) AND ($4::smallint IS NULL OR rating=$4) AND ($5='' OR status::text=$5) ORDER BY created_at DESC LIMIT $6 OFFSET $7`, a.OrganizationID, assistantID, userID, rating, status, limit, (page-1)*limit)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var f Feedback
		if err = rows.Scan(&f.ID, &f.AssistantID, &f.ConversationID, &f.MessageID, &f.UserID, &f.Rating, &f.Comment, &f.CorrectedAnswer, &f.Status, &f.CreatedAt, &f.UpdatedAt, &total); err != nil {
			return nil, 0, err
		}
		out = append(out, f)
	}
...
func (s *Service) Get(ctx context.Context, a auth.Actor, id uuid.UUID) (Feedback, error) {
	...
	var f Feedback
	err := s.db.QueryRow(ctx, `SELECT id,assistant_id,conversation_id,message_id,user_id,rating,comment,corrected_answer,status,created_at,updated_at FROM message_feedback WHERE organization_id=$1 AND id=$2`, a.OrganizationID, id).Scan(&f.ID, &f.AssistantID, &f.ConversationID, &f.MessageID, &f.UserID, &f.Rating, &f.Comment, &f.CorrectedAnswer, &f.Status, &f.CreatedAt, &f.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s *Service) Moderate(ctx context.Context, a auth.Actor, id uuid.UUID, status string) (Feedback, error) {
	...
	defer tx.Rollback(ctx)
	var f Feedback
	err = tx.QueryRow(ctx, `UPDATE message_feedback SET status=$3,updated_at=now() WHERE organization_id=$1 AND id=$2 RETURNING id,assistant_id,conversation_id,message_id,user_id,rating,comment,corrected_answer,status,created_at,updated_at`, a.OrganizationID, id, status).Scan(&f.ID, &f.AssistantID, &f.ConversationID, &f.MessageID, &f.UserID, &f.Rating, &f.Comment, &f.CorrectedAnswer, &f.Status, &f.CreatedAt, &f.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
